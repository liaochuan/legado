...
class ChangeSourceResultOptionsWiringTest {
    ...
    fun `result options reuse cached searches without changing the base preference`() {
        ...
        assertTrue(viewModel.contains("preparingSearch.get() > 0 || activeTask?.isActive == true"))
        assertTrue(viewModel.contains("pendingMeasurementRefresh.set(true)"))
        assertTrue(viewModel.contains("pendingMeasurementRefresh.compareAndSet(true, false)"))
        assertTrue(viewModel.contains("invokeOnCompletion(::refreshPendingMeasurements)"))
        assertTrue(viewModel.contains("refreshList(books)"))
